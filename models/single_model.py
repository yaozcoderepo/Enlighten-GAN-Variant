...
class SingleModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if opt.vgg > 0:
            ...
            # load vgg model and set its mode to evaluation (no gradient update)
        ...
        skip = True if opt.skip > 0 else False  # True
        self.netG_A = networks.define_G(opt.input_nc, opt.output_nc,
                                        opt.ngf, opt.which_model_netG, opt.norm, not opt.no_dropout,
                                        # ngf not used here
                                        self.gpu_ids, skip=skip, opt=opt)
        ...
        if self.isTrain:
            ...
            if self.opt.patchD:
                self.netD_P = networks.define_D(opt.input_nc, opt.ndf,
                                                opt.which_model_netD,
                                                opt.n_layers_patchD, opt.norm, use_sigmoid,
                                                self.gpu_ids, True)
        ...
        if self.isTrain:
            self.old_lr = opt.lr  # 1e-4
            self.fake_B_pool = ImagePool(opt.pool_size)  # 50
            ...
            self.optimizer_D_A = torch.optim.Adam(self.netD_A.parameters(), lr=opt.lr,
                                                  betas=(opt.beta1, 0.999))
            if self.opt.patchD:
                self.optimizer_D_P = torch.optim.Adam(self.netD_P.parameters(), lr=opt.lr,
                                                      betas=(opt.beta1, 0.999))
    ...
    def set_input(self, input):
        AtoB = self.opt.which_direction == 'AtoB'  # True
        input_A = input['A' if AtoB else 'B']  # 'A'
        input_B = input['B' if AtoB else 'A']  # 'B'
    ...
    def test(self):
        ...
        if self.opt.noise > 0:
            self.noise = Variable(torch.cuda.FloatTensor(self.real_A.size()).normal_(mean=0,
                                                                                     std=self.opt.noise / 255.))
        ...
        if self.opt.hybrid_loss:
            ...
            if self.opt.patchD_3 > 0:
                for i in range(self.opt.patchD_3):
                    loss_D_P += self.backward_D_basic(self.netD_P, self.real_patch_1[i],
                                                      self.fake_patch_1[i], False)
        ...
        if self.opt.patchD_3 > 0:
            ...
            for i in range(self.opt.patchD_3):
                ...
                self.fake_patch_1.append(
                    self.fake_B[:, :, h_offset_1:h_offset_1 + self.opt.patchSize,
                    w_offset_1:w_offset_1 + self.opt.patchSize])
                self.real_patch_1.append(
                    self.real_B[:, :, h_offset_1:h_offset_1 + self.opt.patchSize,
                    w_offset_1:w_offset_1 + self.opt.patchSize])
                self.input_patch_1.append(
                    self.real_A[:, :, h_offset_1:h_offset_1 + self.opt.patchSize,
                    w_offset_1:w_offset_1 + self.opt.patchSize])
    ...
    def backward_G(self, epoch):
        ...
        if self.opt.patchD:
            ...
            if self.opt.hybrid_loss:  # True
                ...
                loss_G_A += (self.criterionGAN(pred_real_patch - torch.mean(pred_fake_patch),
                                               False) +
                             self.criterionGAN(pred_fake_patch - torch.mean(pred_real_patch),
                                               True)) / 2
                ...
                if self.opt.hybrid_loss:
                    ...
                    loss_G_A += (self.criterionGAN(
                        pred_real_patch_1 - torch.mean(pred_fake_patch_1), False) +
                                 self.criterionGAN(
                                     pred_fake_patch_1 - torch.mean(pred_real_patch_1), True)) / 2

            if not self.opt.D_P_times2:  # opt.D_P_times2 = False
        ...
        if self.opt.vgg > 0:
            ...
            if self.opt.patch_vgg:
                if not self.opt.IN_vgg:  # opt.IN_vgg = False
